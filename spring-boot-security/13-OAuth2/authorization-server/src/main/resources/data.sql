...
insert into client_app_client_authentication_methods (client_app_id,client_authentication_methods) values ((select id from client_app where client_id = 'client'),'client_secret_basic'); -- client auth method
insert into client_app_redirect_uris (client_app_id,redirect_uris) values ((select id from client_app where client_id = 'client'),'https://oauthdebugger.com/debug'); -- a donde van a mandar el auth code para cambiar x un access token
insert into client_app_scopes (client_app_id,scopes) values ((select id from client_app where client_id = 'client'),'openid'); -- scopes = a q tengo permisos con este cliente. authorities = scopes
...
insert into client_app (client_id,client_secret,duration_in_minutes,required_proof_key) values ('my-own-client','$2a$10$QHLrXJ8nlnDlOXEDUnV6iefGNOkKaW9pqJwZcpF.5HrO42PRPzjcO',10,true); -- cliente del mismo propietario del auth server
...
insert into client_app_scopes (client_app_id,scopes) values ((select id from client_app where client_id = 'my-own-client'),'ALL'); -- se le permite todo a este cliente
