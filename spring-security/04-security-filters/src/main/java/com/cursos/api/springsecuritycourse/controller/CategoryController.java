...
public class CategoryController {
    ...
    public ResponseEntity<Category> findOne(@PathVariable Long categoryId){
        ...
        if(category.isPresent()){
            return ResponseEntity.ok(category.get()); // get para obtener el valor del optional
        }
        return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
    }

    @PostMapping
    public ResponseEntity<Category> findOne(@RequestBody @Valid SaveCategory saveCategory){
